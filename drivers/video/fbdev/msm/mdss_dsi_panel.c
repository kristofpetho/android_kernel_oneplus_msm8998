...
int mdss_dsi_panel_get_serial_number(struct mdss_dsi_ctrl_pdata *ctrl)
{
    ...
    if (ctrl->panel_serial_num_cmds.cmd_cnt){
       ...
       if ((ctrl->panel_year_index > len) || (ctrl->panel_mon_index > len) || (ctrl->panel_day_index > len)
            || (ctrl->panel_hour_index > len) || (ctrl->panel_min_index > len)){
...
static int mdss_dsi_panel_on(struct mdss_panel_data *pdata)
{
	...
	if (mdss_dsi_panel_get_adaption_mode(ctrl)){
		mdss_dsi_panel_set_adaption_mode(ctrl, mdss_dsi_panel_get_adaption_mode(ctrl));
	}
			...
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
    ...
    rc = of_property_read_u32(np, "qcom,mdss-dsi-panel-seria-num-year-index", &tmp);
	ctrl_pdata->panel_year_index = (!rc ? tmp : 0);
    rc = of_property_read_u32(np, "qcom,mdss-dsi-panel-seria-num-mon-index", &tmp);
	ctrl_pdata->panel_mon_index = (!rc ? tmp : 0);
    rc = of_property_read_u32(np, "qcom,mdss-dsi-panel-seria-num-day-index", &tmp);
	ctrl_pdata->panel_day_index = (!rc ? tmp : 0);
    rc = of_property_read_u32(np, "qcom,mdss-dsi-panel-seria-num-hour-index", &tmp);
	ctrl_pdata->panel_hour_index = (!rc ? tmp : 0);
	rc = of_property_read_u32(np, "qcom,mdss-dsi-panel-seria-num-min-index", &tmp);
